...
func init() {
	Subcommands["verseline-edit"] = Subcommand{
		...
		Run: func(name string, args []string) bool {
			...
			segments, resolvedTimelinePath, err := verselineLoadTimelineForProject(project, projectPath)
			if err != nil {
				fmt.Printf("ERROR: Could not load timeline: %s\n", err)
				return false
			}

			if err := runVerselineTUI(&project, projectPath, segments, resolvedTimelinePath); err != nil {
				fmt.Printf("ERROR: %s\n", err)
				return false
			}
			return true
...
func runVerselineTUI(project *VerselineProject, projectPath string, segments []VerselineSegment, timelinePath string) error {
	model := newVerselineTUIRoot(project, projectPath, segments, timelinePath)
