        ...
        {browsing ? (
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="px-4 py-4 space-y-3">
              <div>
                <span className="text-xs text-zinc-500 block">ID</span>
                <span className="text-sm text-zinc-200">{selected.id}</span>
              </div>
              <div>
                <span className="text-xs text-zinc-500 block">Family</span>
                <span className="text-sm text-zinc-200">{selected.family}</span>
              </div>
              {selected.files && selected.files.length > 0 && (
                <div>
                  <span className="text-xs text-zinc-500 block mb-1">Files</span>
                  <ul className="space-y-1">
                    {selected.files.map((f) => (
                      <li key={f} className="text-xs text-zinc-400 font-mono truncate bg-zinc-800 rounded px-2 py-1">
                        {f}
                      </li>
                    ))}
                  </ul>
