...
export function AnchorPicker({ value, onChange }: AnchorPickerProps) {
  return (
    <div className="flex flex-col gap-1">
      <label className="text-xs text-zinc-400">Anchor</label>
      <div
        className="relative w-full aspect-video bg-zinc-900 border border-zinc-700 rounded-lg overflow-hidden"
                ...
                <span
                  className={`w-3 h-3 rounded-full border-2 transition-all ${
                    value === anchor
                      ...
                      : "bg-zinc-700 border-zinc-600 group-hover:bg-zinc-500 group-hover:border-zinc-400"
                  }`}
                />
              </button>
            ))
      ...
      <p className="text-xs text-zinc-500">{LABELS[value as Anchor] ?? value}</p>
    </div>
  );
}
