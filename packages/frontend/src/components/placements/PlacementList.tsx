  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-4 py-3 border-b border-zinc-800">
        <h2 className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Placements</h2>
      </div>

      <div className="flex-1 overflow-y-auto">
        {placements.length === 0 ? (
          <p className="px-4 py-6 text-xs text-zinc-600 text-center">No placements yet.</p>
        ) : (
          <ul>
            {placements.map((p) => (
              <li key={p.id}>
                ...
                >
                  <span className="text-sm font-medium text-zinc-200 truncate">{p.id}</span>
