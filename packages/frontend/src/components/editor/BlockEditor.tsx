...
export default function BlockEditor({ block, index, styles, placements, onChange, onRemove }: BlockEditorProps) {
  ...
  return (
    <div className="border border-zinc-700 rounded-lg p-3 bg-zinc-800/50 flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          rows={3}
          className="bg-zinc-900 border border-zinc-600 rounded px-2 py-1.5 text-sm text-white resize-y focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono leading-relaxed"
          placeholder="Block text…"
          spellCheck={false}
        />
        {hasInlineTags && (
          <p className="text-xs text-indigo-400">
            Contains inline style tags (e.g. <code className="bg-zinc-800 px-1 rounded">&lt;styleId&gt;text&lt;/styleId&gt;</code>)
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <select
          ...
          className="bg-zinc-900 border border-zinc-600 rounded px-2 py-1.5 text-sm text-white focus:outline-none focus:ring-1 focus:ring-indigo-500"
        >
      ...
      {/* Placement dropdown */}
      <div className="flex flex-col gap-1">
        <label className="text-xs text-zinc-500">Placement</label>
        <select
