...
func init() {
	Subcommands["verseline-validate"] = Subcommand{
		...
		Run: func(name string, args []string) bool {
			...
			resolvedTimelinePath := resolveReelPath(filepath.Dir(projectPath), string(project.Timeline))
...
var verselineFieldHints = map[string]string{
	...
	"timeline":        `expected string: "timeline": "timeline.jsonl"`,
...
func validateVerselineProject(project VerselineProject) error {
	...
	if strings.TrimSpace(project.Timeline) == "" {
		return fmt.Errorf("timeline is required (path to a JSONL timeline file)")
	}
