...
type verselineMCPProjectSummary struct {
	...
	Timeline       verselineMCPTimelineInfo   `json:"timeline"`
}
...
The project.json schema:
{
  ...
  "timeline": "string — path to a JSONL timeline file"
}

A timeline JSONL file has one segment per line:
{"id":"string","start":"HH:MM:SS.mmm","end":"HH:MM:SS.mmm","blocks":[{"text":"string","style":"style-id","placement":"placement-id"},...]}
...
Required: canvas (width>0, height>0, fps>0), assets.background.path, timeline.
All arrays use objects with "id" fields, NOT objects keyed by id. Timestamps use HH:MM:SS.mmm format.`,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: `Return paginated segments from a project's timeline. Each segment includes its 1-based number, start/end timestamps, block count, a text preview (first 160 characters), and source references. Defaults: start_at=1, limit=50.
...
Segments can overlap in time — multiple segments may cover the same time range and all render simultaneously as stacked layers. This is by design: use overlapping segments for parallel text cards (e.g., verse text from 0s-20s and a continuous attribution from 0s-60s spanning multiple verses).`,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: `Validate a project JSON file and its timeline. Checks: canvas dimensions are positive, background path exists, all IDs are unique and non-empty, every timestamp parses as HH:MM:SS.mmm, every block has text or a source, and all style/placement/source references in the timeline exist in the project. Returns valid=true or an error describing the first failing check.`,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: `Update properties of a single segment in the timeline. Can set start, end, status, notes, or a single block's text/style/placement. Identify the segment by 1-based segment_number or segment_id. Validates the full timeline after the edit. Set dry_run=true to preview the change without saving. Block text may contain inline style tags: <styleID>text</styleID> to render portions with a different style's color (the styleID must exist in the project's styles array — use verseline_update_project to add it first).
...
func verselineMCPInspectProjectTool(_ context.Context, _ *mcp.CallToolRequest, in verselineMCPInspectProjectInput) (*mcp.CallToolResult, verselineMCPProjectSummary, error) {
	...
	output := verselineMCPProjectSummary{
		...
		Timeline: verselineMCPDescribeTimeline(project, absProjectPath),
	}
...
func verselineMCPListSegmentsTool(_ context.Context, _ *mcp.CallToolRequest, in verselineMCPListSegmentsInput) (*mcp.CallToolResult, verselineMCPListSegmentsOutput, error) {
	_, absProjectPath, segments, timelinePath, err := verselineMCPLoadProjectTimeline(in.ProjectPath)
	if err != nil {
	...
	summary := fmt.Sprintf("Listed %d of %d timeline segments from %s", len(items), len(segments), filepath.Base(absProjectPath))
	return verselineMCPTextResult(summary), output, nil
}
...
func verselineMCPValidateProjectTool(_ context.Context, _ *mcp.CallToolRequest, in verselineMCPValidateInput) (*mcp.CallToolResult, verselineMCPValidateOutput, error) {
	project, absProjectPath, segments, timelinePath, err := verselineMCPLoadProjectTimeline(in.ProjectPath)
	if err != nil {
	...
	summary := fmt.Sprintf("Validated %d timeline segments for %s", len(segments), filepath.Base(absProjectPath))
	return verselineMCPTextResult(summary), output, nil
}
...
func verselineMCPUpdateSegmentTool(_ context.Context, _ *mcp.CallToolRequest, in verselineMCPUpdateSegmentInput) (*mcp.CallToolResult, verselineMCPUpdateSegmentOutput, error) {
	...
	summary := fmt.Sprintf("Updated timeline segment %d in %s", index+1, filepath.Base(absProjectPath))
	return verselineMCPTextResult(summary), output, nil
}
...
func verselineMCPSplitSegmentTool(_ context.Context, _ *mcp.CallToolRequest, in verselineMCPSplitSegmentInput) (*mcp.CallToolResult, verselineMCPSplitSegmentOutput, error) {
	...
	summary := fmt.Sprintf("Split timeline segment %d into %d segments in %s", index+1, replacementCount, filepath.Base(absProjectPath))
	return verselineMCPTextResult(summary), output, nil
}
...
func verselineMCPPreviewSegmentTool(_ context.Context, _ *mcp.CallToolRequest, in verselineMCPPreviewInput) (*mcp.CallToolResult, verselineMCPPreviewOutput, error) {
	outputPath, err := verselinePreviewProject(in.ProjectPath, in.SegmentNumber, "", in.OpenPlayer, nil)
	if err != nil {
	...
	summary := fmt.Sprintf("Rendered preview for segment %d at %s", in.SegmentNumber, outputPath)
	return verselineMCPTextResult(summary), output, nil
}
...
func verselineMCPReadabilityTool(_ context.Context, _ *mcp.CallToolRequest, in verselineMCPReadabilityInput) (*mcp.CallToolResult, verselineMCPReadabilityOutput, error) {
	project, absProjectPath, segments, _, err := verselineMCPLoadProjectTimeline(in.ProjectPath)
	if err != nil {
...
func verselineMCPLoadProjectTimeline(projectPath string) (VerselineProject, string, []VerselineSegment, string, error) {
	project, absProjectPath, err := loadVerselineProject(projectPath)
	if err != nil {
		return project, "", nil, "", err
	}

	segments, timelinePath, err := verselineLoadTimelineForProject(project, absProjectPath)
	if err != nil {
		return project, "", nil, "", err
	}

	return project, absProjectPath, segments, timelinePath, nil
}

func verselineMCPDescribeTimeline(project VerselineProject, absProjectPath string) verselineMCPTimelineInfo {
	info := verselineMCPTimelineInfo{}
	projectDir := filepath.Dir(absProjectPath)

	target := strings.TrimSpace(project.Timeline)
	if target == "" {
