...
func verselineOverlayPosition(placement VerselinePlacement) string {
	...
	case "middle_left":
		return fmt.Sprintf("x=%d:y=(main_h-overlay_h)/2+%d", marginX, marginY)
	case "middle_center", "center":
		return fmt.Sprintf("x=(main_w-overlay_w)/2+%d:y=(main_h-overlay_h)/2+%d", marginX, marginY)
	case "middle_right":
	...
	case map[string]interface{}:
		...
		if ok {
			...
			if !ok {
				...
				return fmt.Errorf("source entry %q has no recognized text field (available string fields: %s) — set text_field in the source config", key, strings.Join(available, ", "))
			}
			result[key] = text
			return nil
		}
